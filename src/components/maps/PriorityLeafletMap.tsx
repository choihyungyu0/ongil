...
import { Circle, CircleMarker, MapContainer, Polygon, Polyline, Popup, Tooltip, ZoomControl } from 'react-leaflet';
...
const seaPolygon: LatLngExpression[] = [
  [35.045, 129.045],
  [35.05, 129.19],
  [35.205, 129.19],
  [35.19, 129.13],
...
export function PriorityLeafletMap() {
  ...
  return (
    <div
      ...
      role="region"
      aria-label="부산 위험구간 우선순위 Leaflet 모의 지도"
    >
      <MapContainer
        center={center}
        zoom={11}
      ...
      >
        ...
        <Polygon
          positions={seaPolygon}
          pathOptions={{ color: '#8ecae6', fillColor: '#d7effa', fillOpacity: 0.58, opacity: 0.32, weight: 1 }}
          interactive={false}
        />
      ...
      <div className="pointer-events-none absolute left-3 top-3 rounded-full border border-white/80 bg-white/90 px-2.5 py-1 text-[9px] font-black text-navy-800 shadow-sm">
        Leaflet 모의 지도
      </div>
      <div className="pointer-events-none absolute bottom-3 left-3 flex flex-wrap gap-1.5">
        {[
          ...
          <span key={label} className="inline-flex items-center gap-1 rounded-full bg-white/90 px-2 py-1 text-[9px] font-black text-slate-600 shadow-sm">
